...
function TablePaginationActions(props: TablePaginationActionsProps) {
  const {
    count: quantidade, page: pagina, rowsPerPage: linhasPorPagina, onChangePage,
  } = props;
  ...
  const ultimaPaginaButtonClickHandler = (event: any) => {
    onChangePage(event, Math.max(0, Math.ceil(quantidade / linhasPorPagina) - 1));
  };
  ...
  return (
    <>
      ...
      <IconButton onClick={paginaAnteriorButtonClickHandler} disabled={pagina === 0}>
        <KeyboardArrowLeft />
      </IconButton>
      <IconButton
