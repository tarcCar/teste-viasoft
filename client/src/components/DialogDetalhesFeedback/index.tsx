...
const ModalDetalhesFeedback: React.FC<ModalDetalhesFeedbackProps> = ({
  ...
  return (
    <Dialog fullWidth maxWidth="md" open={open} onClose={handleClose}>
      <DialogTitle>
        <Typography variant="h5">Detalhes do Feedback</Typography>
      </DialogTitle>
      <DialogContent>
        ...
        <CampoModalDetalhesFeedback
          ...
          valor={
            feedback.criadoEm
              ? format(parseISO(feedback.criadoEm), "dd/MM/yyyy HH:mm:ss")
              : ""
          }
        />
        <CampoModalDetalhesFeedback
          ...
          valor={
            feedback.alteradoEm
              ? format(parseISO(feedback.alteradoEm), "dd/MM/yyyy HH:mm:ss")
              : ""
          }
        />
        {feedback.pontosManter && feedback.pontosManter.length >= 0 && (
          <Grid container className={styles.linha}>
            <Grid item xs={12}>
              <div className={styles.label}>Pontos a Manter:</div>
            </Grid>

            {feedback.pontosManter.map((p) => (
              <Grid item xs={12} md={4} style={{ margin: 10 }}>
                <div className={styles.valor}>{`- ${p.descricao}`}</div>
              </Grid>
            ))}
          </Grid>
        )}
        {feedback.pontosMelhorar && feedback.pontosMelhorar.length >= 0 && (
          <Grid container className={styles.linha}>
            <Grid item xs={12}>
              <div className={styles.label}>Pontos a Melhorar:</div>
            </Grid>
