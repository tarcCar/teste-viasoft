...
const InputPontoMelhorar: React.FC<InputPontoMelhorarProps> = ({
  ...
  const onChangeHandler = (newValue?: PontoMelhorar) => {
    if (newValue) {
      ...
      if (onPontosMelhorarSelecionados)
        onPontosMelhorarSelecionados(novosPontos);
    }
  };
  ...
  return (
    <>
      <PontoMelhorarAutoComplete
        onPontoMelhorarSelected={onChangeHandler}
        error={!!errorText}
      />
      {errorText && (
        <FormHelperText
