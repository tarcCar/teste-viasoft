...
export const PontoManterAutoComplete: React.FC<PontoManterAutoCompleteProps> = ({
  onPontoManterSelected,
  error,
  ...props
}) => {
  const [value, setValue] = useState<PontoManterFilter | null>(null);

  const pontosManter = useSelector(
    (state: RootState) => state.pontoManterReducer.pontosManter
  );
      ...
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField
            ...
            placeholder="Digite para procurar ou adicionar um ponto a manter"
            fullWidth
            error={error}
            variant="outlined"
          />
