...
const setLoginSucesso = (login: Login): LoginActionsTypes => ({
  type: "LOGIN_SUCESSO",
  login,
});
const setLoginErro = (error: string): LoginActionsTypes => ({
  type: "LOGIN_ERRO",
  error,
});
const setLoginLoading = (loading: boolean): LoginActionsTypes => ({
  type: "LOADING_lOGIN",
  loading,
});

const loginAction = (login: string, senha: string): any => async (
  dispatch: any
) => {
  try {
