...
const setGetPontosManter = (
  pontosManter: PontoManter[]
): PontoMAnterActionsTypes => ({ type: "GET_PONTOS_MANTER", pontosManter });
const setGetPontosManterErro = (error: string): PontoMAnterActionsTypes => ({
  type: "GET_PONTOS_MANTER_ERROR",
  error,
});
const setGetPontosManterLoading = (
  loading: boolean
): PontoMAnterActionsTypes => ({ type: "LOADING_GET_PONTOS_MANTER", loading });
