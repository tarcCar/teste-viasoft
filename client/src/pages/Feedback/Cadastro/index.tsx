...
import {
  saveFeedbacksAction,
  clearFeedbackAction,
} from "../../../store/actions/feedback/feedbackActions";
...
const CadastroFeedback: React.FC = () => {
  ...
  const loading = useSelector(
    (state: RootState) => state.feedbackReducer.loadingSaveFeedback
  );
  const erroSaveUsuario = useSelector(
    (state: RootState) => state.feedbackReducer.erroSaveFeedback
  );
  const feedback = useSelector(
    (state: RootState) => state.feedbackReducer.feedback
  );
  ...
  useEffect(() => {
    if (location.state?.feedBackParaAtualizar) {
      const feedBackParaAtualizar = location.state
        .feedBackParaAtualizar as Feedback;
      if (formRef.current) {
        const form = formRef.current;
        form.setFieldValue("id", feedBackParaAtualizar.id);
        form.setFieldValue(
          "pontosManter",
          feedBackParaAtualizar.pontosManter,
          false
        );
        form.setFieldValue(
          "pontosMelhorar",
          feedBackParaAtualizar.pontosMelhorar,
          false
        );
        form.setFieldValue("sugestoes", feedBackParaAtualizar.sugestoes, false);
        form.setFieldValue(
          "usuarioDestino",
          feedBackParaAtualizar.usuarioDestino,
          false
        );
        form.setFieldValue(
          "feedBackFinal",
          feedBackParaAtualizar.feedBackFinal,
          false
        );
        form.validateForm();
      }
          ...
          <Formik
            ...
            validate={(values) => {
              ...
              if (!values.pontosMelhorar || values.pontosMelhorar.length <= 0) {
                errors.pontosMelhorar =
                  "Selecionar pelo menos um ponto a melhorar";
              }
              ...
              <form className={classes.form} noValidate onSubmit={handleSubmit}>
                <Paper className={classes.paper}>
                  ...
                  <SelectUsuario
                    ...
                    error={
                      errors.usuarioDestino
                        ? false
                        : true && touched.usuarioDestino
                    }
                  ...
                  <TextField
                    ...
                    error={
                      errors.feedBackFinal
                        ? true
                        : false && touched.feedBackFinal
                    }
